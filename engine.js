...
// type="module" 전환 — flow-translate.js 분리 반영
// ============================================================
...
import {
    ...
    getDictMeaningMap,
    getMaxPhraseLength,
} from '/public/js/core/flow-translate.js';
...
function saveChatLog(entry) {
    const key  = getTodayKey(window.currentRoom?.id || null);
...
function clearTodayChat() {
    ...
    localStorage.removeItem(getTodayKey(window.currentRoom?.id || null));
    history.innerHTML = '';
    msgCount    = 0;
    firstLang   = null;
    sessionLogs = [];
...
function calcEmotionScore(text) {
    let score = 0;
    ['왜','짜증','싫어','됐어','몰라','하지마','그만'].forEach(w => { if (text.includes(w)) score += 2; });
    ['고마워','사랑해','괜찮아','미안해'].forEach(w => { if (text.includes(w)) score -= 1; });
...
function renderWelcomeCard(vi, ko) {
    ...
    const modeTag = currentMode === 'CHAT' ? `<div class="welcome-mode-tag">● CHAT MODE</div>` : '';
...
function showWelcomeScreen() {
    ...
    initEngine().then(() => {
        ...
        const phrases = dict.filter(d => d.entry_type === 'phrase' || (d.standard?.split(' ').length > 1));
...
// ── window 노출 — onclick에서 직접 호출하는 함수 2개만 ───────
...
window.translateChatMsg = async function(btn, isKorean) {
...
function switchToChatMode(room) {
    ...
    if (roomLogs.length > 0) restoreChat(roomLogs);
...
async function sendChatOnly(text, isKorean, isLeft, tempId, pairDiv) {
    ...
    try {
        ...
        saveChatLog({ original: text, translated: text, topHtml: cardHtml, isKorean, isLeft, checkText: text, firstLang, mode: 'CHAT', timestamp: Date.now() });
    } catch (e) {
...
async function handleChatMode(text, mw, tempId, pairDiv, isKorean, isLeft) {
    try {
        const { translated, softTone } = await translateChat(text, isKorean, {
...
async function handleSend() {
    ...
    try {
        ...
        // emotion / tone — 현재 null, Phase 2에서 활성화
...
function showModal(original, translated, isKorean, cardText) {
    ...
    const dictMap     = getDictMap();
    const meaningMap  = getDictMeaningMap();
    ...
    words.forEach(word => {
        ...
        if (!cleanWord) return;
        const found = dictMap.get(cleanWord.toLowerCase()) || meaningMap.get(cleanWord.toLowerCase());
    ...
    detectConflicts(cardText, getConflictDictionary()).forEach(item => {
        ...
        chunkHtml += `<div class="chunk-card conflict-card"><span class="chunk-v">${icon} ${item.word}</span><span class="chunk-north">북부: ${item.meaning_northern}</span><span class="chunk-south dialect-diff">남부: ${item.meaning_southern}</span>${item.note ? `<span class="chunk-k">${item.note}</span>` : ''}</div>`;
    });
