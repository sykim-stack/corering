...
// type="module" 전환 + flow-translate.js 연동
// window 바인딩: 기존 전부 유지 + translateChatMsg, copyRoomCode 추가
// ============================================================

import { translateRing, translateChat } from '/js/core/flow-translate.js';

// ── 전역 참조 (기존 script 태그로 로드된 파일들) ─────────────
// dialect.js    → detectDialectScore, resolveDialect
// conflict.js   → detectConflicts
// logger.js     → saveTranslationLog
// mindworld.js  → runMindWorld, normalizeEmotion
// pipeline.js   → autoSaveToDataset
// rooms.js      → toggleRooms, exitChatMode, switchToChatMode 등
...
async function handleChatMode(text, mw, tempId, pairDiv, isKorean, isLeft) {
    try {
        // flow-translate.js의 translateChat 사용
        const { rawTranslation } = await translateChat(
            text,
            isKorean,
            sessionLogs.slice(-5),
            mw.softTone,
        ...
        if (mw.softTone) topHtml += ' <span class="conflict-badge risk-medium">💛 순화됨</span>';
...
async function handleSend() {
    ...
    // ── RING 모드 — flow-translate.js의 translateRing 사용 ──
    try {
        const { rawTranslation } = await translateRing(text, isKorean);
...
// ══════════════════════════════════════════════════════════════
// window 바인딩 — 인라인 onclick 및 외부 스크립트에서 호출하는
// 함수 전부 노출. type="module" 전환 시 전역 스코프에서
// 사라지므로 여기서 명시적으로 등록한다.
// ══════════════════════════════════════════════════════════════
window.handleSend           = handleSend;
window.clearTodayChat       = clearTodayChat;
window.toggleMode           = toggleMode;
window.showModal            = showModal;
window.buildChatCard        = buildChatCard;
window.appendChatToHistory  = appendChatToHistory;
window.switchToChatMode     = switchToChatMode;
window.switchToRingMode     = switchToRingMode;
window.updateRoomBar        = updateRoomBar;
window.saveRoomState        = saveRoomState;
window.loadRoomState        = loadRoomState;

// ── 신규 추가 (v4.1) ──────────────────────────────────────────
window.translateChatMsg     = translateChatMsg;  // buildChatCard 내 onclick="translateChatMsg(...)"
window.copyRoomCode         = copyRoomCode;      // room-bar의 onclick="copyRoomCode()"